    ...
    this.getRootPane().addComponentListener(new ComponentAdapter() {
      public void componentResized(ComponentEvent e) {
        if (System.getProperty("os.name").contains("OS X")) {
          //System.out.println("ScreenCapturer: Width: " + (getWidth() - 10) + " Height: " + (getHeight() - 32) + " Proportion: " + (float) (getWidth() - 10) / (float) (getHeight() - 32));

        } else {
          if (MouseInfo.getPointerInfo().getLocation().x < getLocation().x + getWidth() - 50   && MouseInfo.getPointerInfo().getLocation().y > getLocation().y + getHeight() - 50) {
            // bottom resized
           setSize((int) (getHeight() * windowRatio) + 10, getHeight() + 33);
          } else if (
          MouseInfo.getPointerInfo().getLocation().x > getLocation().x + getWidth() - 50   && MouseInfo.getPointerInfo().getLocation().y < getLocation().y + getHeight() - 50) {
            // right resized
            setSize((int) getWidth() + 10, (int) (getWidth() / windowRatio) + 33);
          }

          //System.out.println("ScreenCapturer: Width: " + (getWidth() - 10) + " Height: "+ (getHeight() - 33) + " Proportion: " + (float) (getWidth() - 10) / (float) (getHeight() - 33));
        }
